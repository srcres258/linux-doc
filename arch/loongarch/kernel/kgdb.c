...
char *dbg_get_reg(int regno, void *mem, struct pt_regs *regs)
{
	...
	if ((regs->csr_euen & CSR_EUEN_LBTEN)) {
		...
		switch (regno) {
		case DBG_FTOP:				/* Process the fcc */
			memcpy(mem, (char *)&current->thread.fpu.ftop, reg_size);
			break;
		case DBG_EFLAGS:			/* Process the fcsr */
			memcpy(mem, (void *)&current->thread.lbt.eflags, reg_size);
			break;
...
int dbg_set_reg(int regno, void *mem, struct pt_regs *regs)
{
	...
	if ((regs->csr_euen & CSR_EUEN_LBTEN)) {
		...
		case DBG_FTOP:				/* Process the ftop */
			...
			break;
		case DBG_EFLAGS:			/* Process the eflags */
			memcpy((void *)&current->thread.lbt.eflags, mem, reg_size);
			break;
		case DBG_SCR_BASE ... DBG_SCR_END:	/* Process the scr */
